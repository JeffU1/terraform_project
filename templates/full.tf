...
resource "aws_security_group" "jeff_terra_alb_sg" {
  name        = "jeff_terra_alb_sg"
...
resource "aws_alb_target_group" "terra_alb_target_group" {
  name     = "jeff-terra-alb-target-group"
  port     = 80
  protocol = "HTTP"
  vpc_id   = "${aws_vpc.terra_vpc.id}"
...
resource "aws_alb" "terra_alb" {
  ...
  security_groups    = ["${aws_security_group.jeff_terra_alb_sg.id}"]
  ...
  depends_on = [
    "aws_security_group.jeff_terra_alb_sg",
    "aws_subnet.terra_ingress_subnet_az_1",
